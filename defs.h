...
#define assert(x)                                        \
    do {                                                 \
        if (!(x))                                        \
            panic("assertion failed" _assert_occurs #x); \
    } while (0)
...
uint16_t cksum16(uint16_t const* data, uint16_t size, uint32_t init);
...
struct queue_entry* queue_try_pop(struct queue_head* queue);
...
struct netif* ip_netif_alloc(ip_addr_t unicast, ip_addr_t netmask, ip_addr_t gateway, net_ns_t* net_ns);
struct netif* ip_netif_register(struct netdev* dev, const char* addr, const char* netmask, const char* gateway, net_ns_t* net_ns);
...
ssize_t ip_tx(struct netif* netif, net_ns_t* nns, uint8_t protocol, const uint8_t* buf, size_t len, const ip_addr_t* dst);
